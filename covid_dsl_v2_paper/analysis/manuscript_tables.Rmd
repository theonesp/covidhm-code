...
```{r}
patient<-run_query(getSQL('../sql/patient.sql'))
```
...
table1_printed<-as.data.frame(print(table1
                                    , nonnormal = c('age',"temp_first_emerg","hr_first_emerg","glu_first_emerg","sat_02_first_emerg","bp_max_first_emerg","bp_min_first_emerg","hr_last_emerg","temp_last_emerg","glu_last_emerg","sat_02_last_emerg","bp_max_last_emerg","bp_min_last_emerg","icu_days")
                                    ,missing = T
                                    ,minMax = F
...
```{r}
hosp_adm_icd<-run_query(getSQL('../sql/hosp_adm_icd.sql'))
er_adm_icd<-run_query(getSQL('../sql/er_adm_icd.sql'))
```
...
table1 <- CreateTableOne(vars = allvars
                         , data = hosp_adm_icd%>%filter(abbrev_desc %in% top_er_adm_icd)
                         , factorVars = catvars
                         )
...
```{r}
comorbidities_hosp_adm<-run_query(getSQL('../sql/comorbidities_hosp_adm.sql'))
```
